import { EventManager } from 'react-ogl'
import { createEvents } from './createEvents'

// Basically a copy of this: https://github.com/pmndrs/react-ogl/blob/main/src/events.ts
// Just registering th events on #root instead of canvas
  ...
  connect(canvas, state) {
    ...
    for (const key in EVENTS) {
      ...
      if (handler) {
        ...
        document
          .querySelector('#root')!
          .addEventListener(key, handler, { passive })
      }
    }
  ...
  disconnect(canvas, state) {
    ...
    for (const key in EVENTS) {
      ...
      if (handler) {
        document
          .querySelector('#root')!
          .removeEventListener(key, handler as any)
      }
    }
